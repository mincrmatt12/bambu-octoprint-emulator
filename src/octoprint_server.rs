...
async fn handle_gcode_download(
    ...
    if PathBuf::from(path).normalize().components().ne(gcode.meta.source.targeted_filename().components().filter(|x| !x.is_root())) {
        return StatusCode::NOT_FOUND.into_response();
    }
    ...
    Response::builder()
        ...
        .body(axum::body::Body::from(axum::body::Bytes::from_owner(gcode.content.clone())))
        .unwrap()
}
...
pub async fn serve_octoprint(
    ...
    let api_job_refresher =
        refresh_api_job(event_stream_in, gcode_stream_in, &state.current_job_response);
